...
def compare(df_ay: pd.DataFrame, df_vs: pd.DataFrame) -> dict:
    ...
    df_ay 欄位: area, ticket, price
    df_vs 欄位: area, ticket_vs, ticket_std, price
    """
    df_ay = df_ay.copy()
    ...
    ay_m = ay_m.copy()
    ay_m['ticket_std'] = ay_m['ticket_std'].astype(object)
    vs_m = vs_m.copy()
    vs_m['ticket_std'] = vs_m['ticket_std'].astype(object)

    merged = pd.merge(ay_m, vs_m, on=['area', 'ticket_std'],
                      how='outer', indicator=True)

    both    = merged[merged['_merge'] == 'both'].drop(columns='_merge').copy()
    only_ay = merged[merged['_merge'] == 'left_only'][
        ['area', 'ticket_ay', 'ticket_std', 'price_ay']].copy()
    only_vs = merged[merged['_merge'] == 'right_only'][
        ['area', 'ticket_vs', 'ticket_std', 'price_vs']].copy()
...
def generate_excel(result: dict) -> bytes:
    """將比對結果產出為 Excel bytes（in-memory）。"""
    wb = Workbook()
